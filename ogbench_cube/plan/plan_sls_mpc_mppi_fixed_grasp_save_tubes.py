...
def make_mppi_rollout_and_eval(
    ...
    *,
    ...
    def eval_fn(states, acts, reach_config=None, aux=None, *args, **kwargs):
        ...
        total_cost = jnp.sum(stage_costs + action_costs + state_region_costs + obstacle_costs, axis=-1) + terminal_costs
...
def main():
    ...
    controller = GenericMPC(
        SLSConfig(max_sls_iterations=1, enable_fastsls=True, initialize_nominal=True, warm_start=True, rti=True),
        ...
        cost=cost,
        num_constraints=2 * control_dim + (1 if state_region_constraint is not None else 0) + (1 if obstacle_constraint is not None else 0),
    ...
    for step_idx in mpc_pbar:
        ...
        tube_widths_np, _ = extract_state_tube_widths(solver_info, cfg.horizon + 1, state_dim)
        tube_plan_steps.append(int(step_idx))
        tube_solver_statuses.append(str(status))
        ...
        tube_mppi_centers.append(np.asarray(X_ws, dtype=np.float64))
    ...
    np.savez_compressed(
        ...
        start_state=np.asarray(current_state if not executed_states else executed_states[0], dtype=np.float64),
        ...
        control_timestep=np.asarray(1.0 / 20.0, dtype=np.float64),
    )
    summary_payload = {
        "metadata": {
            "run_dir": str(out_dir),
            ...
            "artifacts": {
                ...
                "tube_data_path": str(out_dir / "tube_data.npz"),

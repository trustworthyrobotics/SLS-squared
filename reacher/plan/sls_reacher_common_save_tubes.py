...
def run_planner(*, use_mppi: bool) -> None:
    ...
    np.savez_compressed(
        ...
        executed_markov_states=np.stack(executed_states, axis=0) if executed_states else np.empty((0, state_dim)),
        ...
        episode_idx=np.asarray(run_index, dtype=np.int64),
        control_timestep=np.asarray(mpc_dt, dtype=np.float64),
    )

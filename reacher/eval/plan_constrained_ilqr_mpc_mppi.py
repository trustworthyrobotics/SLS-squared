...
class PlanSLSRopeConfig:
    ...
    state_abs_limit: float = field(default=2.5)
    state_delta_abs_limit: float = field(default=0.5)
...
def main():
    ...
    x_min = -float(cfg.state_abs_limit) * jnp.ones(state_dim)
    x_max = float(cfg.state_abs_limit) * jnp.ones(state_dim)
    # Keep the finite-difference half of the Markov state tightly bounded.
    x_min = x_min.at[x_min.shape[0]//2:].set(-float(cfg.state_delta_abs_limit))
    x_max = x_max.at[x_max.shape[0]//2:].set(float(cfg.state_delta_abs_limit))

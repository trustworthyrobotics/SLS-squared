...
def load_episode_standalone(dataset_path, episode_idx, args):
    ...
    # Insert this after: dataset = LeWMReacherDataset(...)

    # print(f"\n--- Inspecting Dataset Attributes (Episode {episode_idx}) ---")
    # # List all names (methods and attributes)
    # attributes = dir(dataset)
    # print(f"All available names: {attributes}")

    # # Specifically check for common normalization keywords
    # norm_attrs = [a for a in attributes if "mean" in a or "std" in a or "stats" in a]
    # print(f"Potential normalization attributes found: {norm_attrs}")

    # # If you want to see the actual values of everything stored in the object
    # # Note: This might be large if it contains the full data buffers
    # # print(f"Object dictionary: {vars(dataset).keys()}")
    # print("-------------------------------------------------------\n")
    ...
    with h5py.File(dataset_path, "r") as h5:
        ...
        # print(f"Loaded episode {episode_idx} with {pixels.shape[0]} frames, pixel stats: mean={p_mean}, std={p_std}")
...
def extract_errors(model, pixels, actions, args, device):
    ...
    for step in range(rollout_steps):
        ...
        targets.append(true_latents[t_curr + 1])
    ...
    pred_z = pred_s[..., :true_latents.shape[-1]]
    
    return {"x_t": s_tsr.cpu(), "a_t": a_tsr.cpu(), "error": (target_tsr - pred_z).cpu()}

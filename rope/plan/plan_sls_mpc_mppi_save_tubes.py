...
def load_torch_object_checkpoint(checkpoint_path: Path, requested_device: torch.device) -> tuple[torch.nn.Module, torch.device]:
    checkpoint_path = checkpoint_path.expanduser().resolve()
    model = torch.load(checkpoint_path, map_location="cpu", weights_only=False).eval()
    if requested_device.type != "cuda":
        return model.to(requested_device), requested_device
    try:
        return model.to(requested_device), requested_device
    except (torch.OutOfMemoryError, RuntimeError) as exc:
        if not isinstance(exc, torch.OutOfMemoryError) and "CUDA" not in str(exc):
            raise
        print(
            "CUDA was unavailable while moving the Rope dynamics model. "
            "Falling back to CPU for this visualization run."
        )
        try:
...
def load_error_model_checkpoint(checkpoint_path: Path, requested_device: torch.device) -> tuple[torch.nn.Module, torch.device]:
    error_model = MGNLLPredictor.load_from_checkpoint(checkpoint_path, map_location="cpu").eval()
    if requested_device.type != "cuda":
        return error_model.to(requested_device), requested_device
    try:
        return error_model.to(requested_device), requested_device
    except (torch.OutOfMemoryError, RuntimeError) as exc:
        if not isinstance(exc, torch.OutOfMemoryError) and "CUDA" not in str(exc):
            raise
        print(
            "CUDA was unavailable while moving the Rope error model. "
            "Falling back to CPU for this visualization run."
        )
        try:
            torch.cuda.empty_cache()
            ...
            pass
        return error_model.cpu(), torch.device("cpu")
...
class PlanSLSRopeConfig:
    ...
    save_full_tube_response: bool = field(
        default=False,
        metadata={"help": "Also save full Phi_x response tensors. This can make tube_data.npz much larger."},
    )
...
def extract_state_tube_widths(solver_info: list, horizon_len: int, state_dim: int) -> tuple[np.ndarray, np.ndarray | None]:
    """Project SLS state response tensors to per-dimension tube half-widths."""
    tube_widths = np.full((horizon_len, state_dim), np.nan, dtype=np.float64)
    if len(solver_info) < 3:
        return tube_widths, None

    phi_x = np.asarray(solver_info[2], dtype=np.float64)
    if phi_x.ndim < 3:
        return tube_widths, phi_x

    projected = np.asarray(jnp.linalg.norm(jnp.asarray(phi_x), ord=2, axis=-1).sum(axis=1), dtype=np.float64)
    usable_horizon = min(horizon_len, projected.shape[0])
    usable_dim = min(state_dim, projected.shape[-1])
    tube_widths[:usable_horizon, :usable_dim] = projected[:usable_horizon, :usable_dim]
    return tube_widths, phi_x
...
def main():
    ...
    model, device = load_torch_object_checkpoint(checkpoint_path, device)
    ...
    if cfg.use_constant_covariance:
        ...
        error_model_torch, device = load_error_model_checkpoint(cfg.error_model_ckpt, device)
    ...
    with mujoco.Renderer(env.model, height=int(pixels_np.shape[1]), width=int(pixels_np.shape[2])) as renderer:
        ...
        tube_plan_steps: list[int] = []
        tube_solver_statuses: list[str] = []
        tube_nominal_centers: list[np.ndarray] = []
        tube_nominal_actions: list[np.ndarray] = []
        tube_widths_by_plan: list[np.ndarray] = []
        tube_mppi_centers: list[np.ndarray] = []
        tube_full_phi_x: list[np.ndarray] = []
        ...
        for step_idx in pbar:
            ...
            nominal_centers_np = np.asarray(X_pred, dtype=np.float64)
            if nominal_centers_np.shape != (cfg.horizon + 1, state_dim):
                padded_centers = np.full((cfg.horizon + 1, state_dim), np.nan, dtype=np.float64)
                usable_horizon = min(padded_centers.shape[0], nominal_centers_np.shape[0])
                usable_dim = min(padded_centers.shape[1], nominal_centers_np.shape[-1])
                padded_centers[:usable_horizon, :usable_dim] = nominal_centers_np[:usable_horizon, :usable_dim]
                nominal_centers_np = padded_centers
            nominal_actions_np = np.atleast_2d(np.asarray(U_pred, dtype=np.float64))
            if nominal_actions_np.shape != (cfg.horizon, action_dim):
                padded_actions = np.full((cfg.horizon, action_dim), np.nan, dtype=np.float64)
                usable_horizon = min(padded_actions.shape[0], nominal_actions_np.shape[0])
                usable_dim = min(padded_actions.shape[1], nominal_actions_np.shape[-1])
                padded_actions[:usable_horizon, :usable_dim] = nominal_actions_np[:usable_horizon, :usable_dim]
                nominal_actions_np = padded_actions
            tube_widths_np, phi_x_np = extract_state_tube_widths(solver_info, cfg.horizon + 1, state_dim)
            tube_plan_steps.append(int(step_idx))
            tube_solver_statuses.append(str(solver_status))
            tube_nominal_centers.append(nominal_centers_np.astype(np.float64))
            tube_nominal_actions.append(nominal_actions_np.astype(np.float64))
            tube_widths_by_plan.append(tube_widths_np.astype(np.float64))
            tube_mppi_centers.append(np.asarray(X_warmstart, dtype=np.float64))
            if cfg.save_full_tube_response and phi_x_np is not None:
                tube_full_phi_x.append(phi_x_np.astype(np.float64))
    ...
    tube_artifacts = {
        "plan_steps": np.asarray(tube_plan_steps, dtype=np.int64),
        "solver_status": np.asarray(tube_solver_statuses),
        "nominal_centers": np.asarray(tube_nominal_centers, dtype=np.float64),
        "nominal_actions": np.asarray(tube_nominal_actions, dtype=np.float64),
        "tube_widths": np.asarray(tube_widths_by_plan, dtype=np.float64),
        "mppi_centers": np.asarray(tube_mppi_centers, dtype=np.float64),
        "executed_markov_states": np.asarray(executed_states, dtype=np.float64),
        ...
        "control_timestep": np.asarray(control_timestep, dtype=np.float64),
    }
    if cfg.save_full_tube_response and tube_full_phi_x:
        tube_artifacts["phi_x"] = np.asarray(tube_full_phi_x, dtype=np.float64)
    np.savez_compressed(run_dir / "tube_data.npz", **tube_artifacts)
    summary_payload = {
        "metadata": {
            "run_dir": str(run_dir),
            ...
            "artifacts": {
                ...
                "tube_data_path": str(run_dir / "tube_data.npz"),

...
def plot_clustered_coverage(metrics, target_coverage):
    ...
    plt.legend()
    plt.savefig("rope/eval/clustered_coverage.png")
...
def plot_ratio_distribution(ratios, alpha):
    ...
    plt.savefig("rope/eval/volume_ratio_distribution.png", dpi=300)
...
if __name__ == "__main__":
    PATH = "rope/eval/rope_one_step_error_data.pt"
    ALPHA = 0.1
    lightning_dir = Path("rope/models/err_pred_logs")
    ...
    model = MGNLLPredictor(
        input_dim=dm.input_dim, state_dim=dm.state_dim, 
        num_layers=2, hidden_dim=256, diagonal=False, 
        lr=0.00025, reg_scale=10, dropout_prob=0.3, use_spectral_norm=False
    )

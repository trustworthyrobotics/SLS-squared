#!/usr/bin/env python3
"""Fit zero-centered 99% latent and Markov-state ellipsoids for Rope data."""
...
DEFAULT_DATASET_PATH = Path("rope/data/test_data_noshadow/rope_random_cubic_spline.h5")
DEFAULT_MODEL_DIR = Path("rope/models/mlpdyn_noshadow_ft")
DEFAULT_OUT_DIR = Path("rope/eval/latent_ellipsoid_noshadow")
...
def fit_zero_centered_ellipsoid(latents: np.ndarray, *, alpha: float, ridge: float) -> dict[str, np.ndarray | float | int]:
    latents = np.asarray(latents, dtype=np.float64)
    if latents.ndim != 2:
        raise ValueError(f"Expected latents with shape [N, D], got {latents.shape}.")
    if latents.shape[0] <= latents.shape[1]:
        raise ValueError(
            f"Need more samples than dimensions to fit a stable ellipsoid, got N={latents.shape[0]}, D={latents.shape[1]}."
        )

    dim = latents.shape[1]
    second_moment = (latents.T @ latents) / float(latents.shape[0])
    covariance = second_moment + float(ridge) * np.eye(dim, dtype=np.float64)
    precision = np.linalg.pinv(covariance, hermitian=True)

    scores = np.einsum("ni,ij,nj->n", latents, precision, latents)
    ...
    aligned_latents = latents @ eigvecs
    aligned_axis_max_abs = np.max(np.abs(aligned_latents), axis=0)
...
def main() -> None:
    ...
    with h5py.File(dataset_path, "r") as h5:
        ...
        for ep_idx in tqdm(episode_indices, desc="Encoding rope latents"):
            ...
            all_markov_states.append(build_all_markov_states(all_latents[-1], markov_deriv))
    ...
    metadata = {
        ...
        "markov_score_definition": "x.T @ markov_precision @ x, where x=[z, delta_z] for markov_deriv=1",
    ...
    print(
        f"Saved zero-centered latent and Markov ellipsoids to {out_dir}. "
